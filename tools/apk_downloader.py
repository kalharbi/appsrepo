...
def run_async(apk_dict, remote_pw, remote_user_name, remote_host_name):
    ...
    custom_file_name = app_info["package_name"] + '-' + app_info["version_code"] + '.apk'   
...
class ApkDownloader(object):
    ...
    def connect_to_ssh(self):
        ...
        self.ssh_client.connect(self.remote_host_name, username=self.remote_user_name, password=self.remote_pw)
    ...
    def document_exists(apk_collection, package_name, version_code):
        ...
        if (apk_collection.find(
            {"metadata.n": package_name, "metadata.verc": version_code}).count() > 0):
            return True
        else:
            return False
    ...
    def get_app_info(self, apk_file):
        ...
        if stdout.channel.recv_exit_status() != 0:
            log.error("Failed to run aapt on %s. %s", apk_file, str(''.join(stderr.readlines())))
            return None
        else:
    ...
    def start_main(self, remote_apk_paths_file):
        ...
        pool = Pool(initializer = init_destination_dir, processes=self.processes, initargs = (files_counter, dir_counter, ))
        ...
        with open(remote_apk_paths_file, 'r') as apk_list_file:            
            for apk_file in apk_list_file:
                ...
                if (self.document_exists(apks_collection, app_info["package_name"], app_info["version_code"])):
                    log.info("APK file for package %s, version code: %s already exists.",
        ...
        if len(apks_dicts) > 0:
            ...
            results = [pool.apply_async(run_async, (apk_dict,
                                        self.remote_pw, self.remote_user_name,
                                        self.remote_host_name))
            ...
            for r in results:
                if(r != None):
                    ...
                    if(destination_apk_file is None or app_info is None):
                        continue
                    log.info("APK file %s-%s has been downloaded at: %s", app_info["package_name"],
                    ...
                    apk_file_doc = {'n': app_info["package_name"] ,
                                    ...
                                    'vern': app_info["version_name"],
                                    'size': self.get_file_size(destination_apk_file),
                                    'path': os.path.abspath(destination_apk_file)}
                    doc_id = apks_collection.insert(apk_file_doc)
                    log.info("A new document (Id: %s) containing metadata for  %s." +
    ...
    def main(self, args):
        ...
        # command line parser
        parser = OptionParser(usage="%prog [options] remote_apk_paths_file local_destination_directory", 
                              version="%prog 1.0",
                              description= '%prog -- Download a list of apk files from a remote server into a local server.' + 
                              '-- Arguments: <remote_apk_paths_file>: a text file that contains the full path names to the apk files at the remote server.'+
                              ' <local_destination_directory>: a directory at the local server at which the apk files are downloaded.')
        ...
        parser.add_option('-b', '--db', dest= 'db_name', 
                          help='The name of MongoDB database to store the apk paths. Default is apps.')
        parser.add_option('-c', '--collection', dest= 'collection_name', 
                          help='The name of the MongoDB collection to store the paths. Default is apks.')
        ...
        if os.path.isdir(args[1]):
            ...
            sys.exit("Error: local destination directory " + args[1] + " does not exist.")
        if not os.path.isfile(args[0]):
            sys.exit("Error: remote apk paths file " + args[0] + " does not exist.")
            
        print('This tool will establish an ssh connection to the remote server.')
        print('Remote server connection information is required to issue rsync commands:')
