...
class LayoutParser(object):
    ...
    def start_parser(self, layout_file, apk_dir):
        ...
        for child in root.iter():
            if child.tag == 'include':
                    ...
                    for k, v in d.items():
                        if '/' in v and '@' in v:
                            ...
                            if pattern is not None:
                                ...
                                if os.path.exists(
                                        os.path.join(apk_dir, 'res', dir_name)):
                                    embedded_layout = v
                                    break
                if embedded_layout is None:
                ...
                embeded_file_full_path = os.path.join(apk_dir, 'res',
                                                      embeded_dir,
                                                      embeded_file + '.xml')
                # Get the tree of the included layout
                emebeded_tree = LayoutParser(self.log).parse(
                    embeded_file_full_path,
                    apk_dir)
                ...
                for attr_name, attr_value in attributes.iteritems():
                    if '@' in attr_value:
                        ...
                        if res_element != 'id' and res_element != '+id':
                            ref_value = self.find_attr_value(res_file,
                                                             res_element,
                                                             res_attr)
    ...
    def find_attr_value(self, xml_file, element_name, attribute_name):
        try:
            ...
            self.log.error("XML parsing error occured while parsing file: %s",
                           xml_file)
        return []
