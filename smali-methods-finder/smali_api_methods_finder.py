...
def run_grep(smali_text_file, search_word):
    ...
    sub_process = Popen(
                ['grep', '-lm 1', search_word, smali_text_file], stdout=PIPE, stderr=PIPE)
    ...
    if rc != 0:
        log.error('%s is not found in: %s. %s', search_word, smali_text_file, err)
...
class SmaliApiMethodsFinder(object):
    ...
    def start_main(self, search_word, source_dir, target_dir):
        ...
        for smali_file in [os.path.join(source_dir, f) for f in os.listdir(source_dir)]:
        ...
        if len(smali_methods_file_list) > 0:
            try:
                ...
                if self.ordered:
                    ...
                    smali_methods_file_list.sort(key=lambda x: os.path.getmtime(x))
                # Run grep on the each smali text file asynchronously.
                results = [pool.apply_async(run_grep, (smali_text_path, search_word)) for smali_text_path in smali_methods_file_list]
                for r in results:
                    if r is not None:
    ...
    def main(self, args):
        ...
        # command line parser
        parser = OptionParser(
            usage="python %prog [options] search_word smali_methods_text_files_dir target_dir",
            version="%prog 1.1",
            description='The smali_api_methods_finder tool recursively searches for '
                        'words in smali text files generated by the smali_api_methods tool.')
